...
function logbug(){
	...
	\Think\Log::write("bug::::::::::::::::::::::::::",'WARN');
	foreach($dbug as $value){
		\Think\Log::write("file:".$value['file'],'WARN');
		\Think\Log::write("line:".$value['line'],'WARN');
		\Think\Log::write("function:".$value['function'],'WARN');
	}
	\Think\Log::write("bug::::::::::::::::::::::::::",'WARN');
}
// 获取当前用户的OpenId
...
function get_token($token = NULL) {
	...
	if(!$GLOBALS['is_wap'] && $reset){
		if(empty($GLOBALS['myinfo'])) $token = -1;
			...
			$sql = 'SELECT public_id FROM `public_link` as l LEFT JOIN public as p on l.mp_id=p.id WHERE l.uid='.$GLOBALS['mid'];
			$list = M()->query($sql);
			$flat = false;
